...
/**
 * todoListList reducer -
 *
 * Accepts arbitrary list arguments and recursively builds nested list as needed
 *
 * NOTE: this is never called directly. Only by parent 'todoList' reducer (defined
 * below) when dealing with a LIST action
 */
function todoListList(state = {
  /**
   ...
   * then recursively return an _additional_ todoListList reducer.
   *
   * Otherwise, return the actual todoList lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional todoListList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: todoListList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_TODO_LIST_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_TODO_LIST_LIST: {
        return {
          ...state
          , error: null
          , filter: state.filter || {}
      ...
      case Actions.RECEIVE_TODO_LIST_LIST: {
        if(!action.success) {
          return {
            ...state
            , error: action.error
      ...
      case Actions.ADD_TODO_LIST_TO_LIST: {
        let idArray = [];
      ...
      case Actions.REMOVE_TODO_LIST_FROM_LIST: {
      ...
      case Actions.SET_TODO_LIST_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_TODO_LIST_PAGINATION: {
        return {
          ...state
...
/**
 * Primary todoList reducer -
 *
 * This is the single source of truth for all things 'todoList' related within the
...
function todoList(state = {
  ...
  /**
   * "byId" is an object map of all todoList items in the store. The map's keys are
   * the Mongo ids of the objects by default
   */
  byId: {}

  /**
   * "defaultItem" defines fields for a _new_ todoList
   * any component that creates a new todoList object should store a copy of this
   * in its state
   */
  , defaultItem: {
    error: null
  ...
  /**
   * "lists" corresponds to individual instances of the todoListList reducer as
   * defined above.
   *
   ...
   *
   * For example, when changing the name of a todoList, the single todoList
   ...
   *
   * For example, when fetching a nested list of todoLists by type and color you
   * would write something like:
   * let list = todoListStore.util.getList('type', 'apparel', 'color', 'black')
   */
  , util: {
    getKeyArrayFromList: () => {
      return null
  ...
  switch(action.type) {
    /**
     * DEFAULT TODO_LIST ACTIONS
     */
    case Actions.REQUEST_DEFAULT_TODO_LIST: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_TODO_LIST: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUEST_TODO_LIST_SCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_TODO_LIST_SCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     */
    case Actions.REQUEST_SINGLE_TODO_LIST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_SINGLE_TODO_LIST: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_TODO_LIST_TO_MAP: {
      // add this todoList to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_TODO_LIST: {
      // add this todoList to the map and set it as selected
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_TODO_LIST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_CREATE_TODO_LIST: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_TODO_LIST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_UPDATE_TODO_LIST: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_TODO_LIST: {
      nextState = {
        ...state
        , selected: {
          error: null
    ...
    case Actions.RECEIVE_DELETE_TODO_LIST: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_TODO_LIST: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_TODO_LIST_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: todoListList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
